...
Instafetch.prototype.fetch = function(params) {
  ...
  $.ajax({
      ...
      success: function(response) {
        ...
        const maxId = arrayMax(response.data.map((item) => item.id))
        const minId = arrayMin(response.data.map((item) => item.id))
        debugger;
        if((~~filteredArr.length) >= params.limit || !(typeof response.pagination.next_max_id !== 'undefined' || minId)) {
          ...
          parent.fetch({
            ...
            maxId: response.pagination.next_max_id || minId,
            tmpArr: filteredArr
          });
        }
      }
